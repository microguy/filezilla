...
static DWORD try_connect(NetSocket *sock)
{
    ...
    if ((
        ...
        err = p_WSAGetLastError();
        /*
         * We expect a potential EWOULDBLOCK here, because the socket is
         * non-blocking and connect() will complete asynchronously.
         */
        if ( err == WSAEWOULDBLOCK || err == WSAEINPROGRESS ) {
            /* Use select() with 3-second timeout to wait for connection */
            ...
            /* Set 3-second timeout for reconnection */
            timeout.tv_sec = 3;
            timeout.tv_usec = 0;
            ...
            if (result == 0) {
                /* Timeout occurred */
                sock->error = "Network error: Connection timed out (3 seconds)";
                err = WSAETIMEDOUT;
                goto ret;
                ...
                /* Connection succeeded - check if really connected */
                int optval;
                int optlen = sizeof(optval);
         ...
         * immediately and we should set the socket as writable.
         */
        sock->writable = true;
        SockAddr thisaddr = sk_extractaddr_tmp(sock->addr, &sock->step);
        plug_log(sock->plug, PLUGLOG_CONNECT_SUCCESS,
